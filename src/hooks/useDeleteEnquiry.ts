...
import { EnquiryResponse as DeleteEnquiryResponse, Enquiry, PaginatedEnquiryResponse } from '@/types/enquiryTypes';
import { deleteEnquiry } from '@/services/enquiryServices';
...
export const useDeleteEnquiry = () => {
  ...
  const deleteMutation = useMutation<
    DeleteEnquiryResponse,     // Type of data returned by the mutationFn
    Error,                  // Type of error
  ...
  >({
    ...
    onMutate: async (enquiryId) => {
      // 1. Cancel any outgoing refetches for 'enquiries' queries.
      // This prevents them from overwriting our optimistic update.
      await queryClient.cancelQueries({ queryKey: ['enquiry'] });

      // 2. Snapshot the previous state of all 'enquiries' queries.
      ...
      queryClient.setQueriesData<PaginatedEnquiryResponse>(
        ...
        (oldData) => {
          ...
          const newEnquiry = oldData.data.filter(
            (enquiry: Enquiry) => enquiry.id !== enquiryId
          );
    ...
    // This is what triggers the refetch of the enquiries list.
    onSettled: () => {
