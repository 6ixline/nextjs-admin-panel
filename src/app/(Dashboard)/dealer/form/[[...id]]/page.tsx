...
export default function UserForm() {
  ...
  const handleNewFilesChange = async (files: File[]) => {
    try {
      ...
      if (response.success) {
        ...
        toast.error("Error! while uploading profile pic");
      }
    } catch (err) {
      toast.error("Error! while uploading profile pic");
    }
  ...
  const handleFileRemoval = (fileId: string | undefined, fileName: string) => {
    ...
    setCurrentExistingFiles(prev =>
      prev.filter(file => {
        if ("fileid" in file) {
          // Existing backend file: match by id
          return (file as ExistingFile).fileid !== fileId;
        }
        // Local File: fall back to name match
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle={id ? "Edit Dealer" : "Add New Dealer"} showNewButton={false} />
